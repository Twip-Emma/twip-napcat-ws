...
def do(user_imgs: List[BuildImage] = UserImgs(1, 2), sender_img: BuildImage = SenderImg()):
    print(user_imgs)
    ...
    if len(user_imgs) < 2:
        ...
        frames: list[IMG] = []
        for i in range(3):
            frame = BuildImage.open(Path(BASE_PATH) / "memes" / "do" / f"{i}.png")
            frame.paste(user_head, user_locs[i], alpha=True)
            frame.paste(self_head, self_locs[i], alpha=True)
            frames.append(frame.image)
        return save_gif(frames, 0.05)
...
def shoot(user_img: BuildImage = UserImg(), sender_img: BuildImage = SenderImg()):
    frames: list[IMG] = []
    img = (
            user_img
            .convert("RGBA")
            .resize((160, 97), keep_ratio=True)
        )
    for i in range(13):
        fluid = BuildImage.open(Path(BASE_PATH) / "memes" / "shoot" / f"{i}.png")
