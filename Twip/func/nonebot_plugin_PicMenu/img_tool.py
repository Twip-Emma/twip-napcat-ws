...
def simple_text(
    ...
    # 新版Pillow使用getbbox()替代getsize()
    bbox = using_font.getbbox(text)  # 返回 (left, top, right, bottom)
    pic_size = (bbox[2] - bbox[0], bbox[3] - bbox[1])  # (width, height)
    
    # 创建透明背景图片
    pic = Image.new("RGBA", pic_size, (0, 0, 0, 0))
    draw = ImageDraw.Draw(pic)
    
    # 调整绘制位置（考虑字符可能存在的悬垂部分）
    draw.text((-bbox[0], -bbox[1]), text, fill=color, font=using_font)
    return pic
        ...
        for line in total_lines:
            ...
            for i, piece in enumerate(line):
                ...
                for cha in piece["text"]:
                    ...
                    bbox = using_font.getbbox(cha)
                    cha_width = bbox[2] - bbox[0]  # right - left
